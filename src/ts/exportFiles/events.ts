...
const exportEventPdfLegacy = (ar: any, start: any, end: any) => {
  // @ts-ignore
...
const eventCoordinates = (event: any): string => {
...
export const exportEventPdf = (events: any[], start: any, end: any) => {
  const clean = (value: any) => String(value ?? '').replace(/\s+/g, ' ').trim();
  const repairEncoding = (value: any): string => {
  ...
  const eventEmoji = (value: any): string => {
    const match = repairEncoding(value).match(/^[\p{Extended_Pictographic}\uFE0F\u200D]+/u);
    return match?.[0] ?? '';
  };
  const eventDescription = (value: any): string => clean(
    repairEncoding(value).replace(/^[\p{Extended_Pictographic}\uFE0F\u200D]+\s*/u, '')
  );
  const userCounts = events.reduce<Record<string, number>>((counts, event) => {
