...
window.downloadXlsx = (rows, title) => {
    const data = Array.isArray(rows) ? rows : [];
    const headers = data.length ? Object.keys(data[0]) : [];
    const cells = (values, row) => values.map((value, column) => `<c r="${columnName(column)}${row}" t="inlineStr"><is><t xml:space="preserve">${xml(value)}</t></is></c>`).join('');
