...
const emojiImage = (emoji) => {
    if (!emoji || emojiImages.has(emoji))
        return emojiImages.get(emoji) ?? '';
    try {
...
export const createModernPdf = ({ title, subtitle, origin, start, end, rows, columns, filename, users = [], summary, evidenceLabel = 'Evidencias', }) => {
    // @ts-ignore
    ...
    let x = margin;
    let y = 10;
    const evidences = rows.flatMap((row, rowIndex) => {
    ...
    rows.forEach((row, index) => {
        ...
        columns.forEach((column, columnIndex) => {
            if (columnIndex)
                doc.line(columnX[column.key], y, columnX[column.key], y + rowHeight);
            ...
            if (emoji) {
                ...
                if (image)
                    doc.addImage(image, 'PNG', columnX[column.key] + 1.2, y + rowHeight / 2 - 2, 3.8, 3.8);
    ...
    if (evidences.length) {
        ...
        evidences.forEach((evidence, index) => {
            ...
            try {
                doc.addImage(evidence.image, 'JPEG', evidenceX + 2, y + 2, cardWidth - 4, imageHeight - 2);
            }
            catch (_) {
                doc.setFillColor(243, 244, 246);
                doc.rect(evidenceX + 2, y + 2, cardWidth - 4, imageHeight - 2, 'F');
            }
            doc.setFillColor(248, 249, 252);
            ...
            if (evidenceX + cardWidth > width - margin || index === evidences.length - 1) {
                evidenceX = margin;
                y += cardHeight + 5;
            }
        });
